...
//-------------PERCEPTRON OR -------------------------\\
	...
	do{
		...
		for(int j = 0 ; j < numEjemplos; j++){
			...
			entrada = entradas[j];
			error_global+= percy.entrenar(entrada,testOR[j],eta,numEntradas);
...
//----------ETA PARA OR---------\\
	...
	float arreta [5] = {0.01,0.1,0.2,0.5,0.99};
		...
		for(int j = 0 ; j < numEjemplos; j++){
			...
			entrada = entradas[j];
			error_global+= percy.entrenar(entrada,testOR[j],arreta[i],numEntradas);
...
//----------------REGLA DE ENTRENAMIENTO OR-----------------------\\
	...
	for(int i =0;i < 5; i++){
		...
		for(int j = 0 ; j < numEjemplos; j++){
			...
			entrada = entradas[j];
			error_global+= jackson.entrenar(entrada,testOR[j],arreta[i],numEntradas);
	...
	cout << "******************\nEntrenando OR valor descendiente de eta en la iesima iteración:\n\n\n";
	for(int i =0;i < 5; i++){
		jackson.resetPesos();
		...
		for(int j = 0 ; j < numEjemplos; j++){
			...
			else error_global+= jackson.entrenar(entrada,testOR[j],arreta[i],numEntradas);
